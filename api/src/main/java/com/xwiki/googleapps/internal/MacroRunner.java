...
/**
 * The environment used to run the driveMacro macros.
 *
 * @version $Id$
 * @since 1.0
 */
...
@Singleton
public class MacroRunner implements Initializable, GoogleAppsConstants
{
    @Inject
    ...
    @Inject
    @Named("googleApps")
    private Provider<IdentityOAuthProvider> googleAppsIOProvider;

    private GoogleAppsConnections gappsConnections;
    ...
    private void initGAConnections()
    {
        if (gappsConnections == null) {
            gappsConnections = (GoogleAppsConnections) (googleAppsIOProvider.get());
        }
    }
    ...
    GoogleAppsConnections.MacroRun runMacro(WikiMacroParameters macroParams)
    {
        initGAConnections();
    ...
    class MacroRun implements GoogleAppsConnections.MacroRun
    {
        ...
        MacroRun(WikiMacroParameters macroParams)
        {
            ...
            authenticationNeeded = gappsConnections.isMissingAuth();
            ...
            if (authenticationNeeded) {
                url = gappsConnections.getOAuthStartUrl();
